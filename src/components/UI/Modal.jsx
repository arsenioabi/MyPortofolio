    ...
    return (
        <div className='w-full h-full fixed top-0 left-0 z-10 bg-headingColor bg-opacity-40'>
            <div className='w-11/12 md:max-w-[600px] md:w-full absolute top-1/2 left-1/2 z-20 bg-white rounded-[8px] transform -translate-x-1/2 -translate-y-1/2 p-5'>
                ...
                <div>
                    <h2 className='text-center text-2xl text-headingColor font-[700] my-5'>{portfolio.title}</h2>
                    <p className='text-justify text-[15px] leading-7 text-smallTextColor'>{portfolio.description}</p>
                    <div className='mt-5 flex items-cemter gap-3 flex-wrap'>
                        <h4 className='text-headingColor text-[18px] text-[700]'>Technologies :</h4>
                        {portfolio.technologies.map((item, index)=> (
                            <span key={index} className='bg-gray-300 py-1 px-2 rounded-[5px] text-[14px] leading-0'>{item}</span>
                        ))}
                    </div>
